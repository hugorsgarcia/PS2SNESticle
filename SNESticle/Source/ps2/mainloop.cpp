...
// PS2SDK cdfs - replaces custom CDVD.IRX
static inline void CDVD_FlushCache() { /* no-op: PS2SDK cdfs handles caching internally */ }

// Embedded IOP modules (linked into ELF via bin2c)
...
static AudsrvMixBuffer *_AudsrvMix;
...
{
	int fd;
	fd = open("host:memdump.bin", O_WRONLY | O_CREAT, 0644);
	if (fd >= 0)
	{
...
// Load an IOP module from an embedded buffer (bin2c data linked into ELF)
static int IOPLoadEmbeddedModule(const char *pName, void *pData, unsigned int size)
{
	int ret, mod_res;
	ret = SifExecModuleBuffer(pData, size, 0, NULL, &mod_res);
	if (ret >= 0)
	{
		ScrPrintf("IOP Embed: %s\n", pName);
	} else
	{
		ScrPrintf("IOP EFail: %s %d\n", pName, ret);
		printf("IOP: Failed to load embedded module '%s' (%d)\n", pName, ret);
	}
	return ret;
}
...
static Bool _MainLoopInitNetwork(Char **ppSearchPaths)
{
	...
	// Return TRUE since networking is embedded and initialized
	return TRUE;
}
...
static void _MainLoopLoadModules(Char **ppSearchPaths)
{
	...
	// ROM modules: SIO2, pad, multitap, memcard
	...
	// Embedded CD/DVD filesystem (PS2SDK)
	IOPLoadEmbeddedModule("cdvdman", cdvdman_irx, size_cdvdman_irx);
	IOPLoadEmbeddedModule("cdvdfsv", cdvdfsv_irx, size_cdvdfsv_irx);
	IOPLoadEmbeddedModule("cdfs",    cdfs_irx,    size_cdfs_irx);

	// Embedded USB mass storage (PS2SDK)
	IOPLoadEmbeddedModule("usbd",          usbd_irx,          size_usbd_irx);
	IOPLoadEmbeddedModule("bdm",           bdm_irx,           size_bdm_irx);
	IOPLoadEmbeddedModule("bdmfs_fatfs",   bdmfs_fatfs_irx,   size_bdmfs_fatfs_irx);
	IOPLoadEmbeddedModule("usbmass_bd",    usbmass_bd_irx,    size_usbmass_bd_irx);

	// Sound driver (from ROM)
	IOPLoadModule("rom0:LIBSD", NULL, 0, NULL);

	// Embedded audsrv module
	if (IOPLoadEmbeddedModule("audsrv", audsrv_irx, size_audsrv_irx) >= 0)
	{
		printf("audsrv_init()\n");
		if (audsrv_init() != 0)
			printf("Could not initialize audsrv\n");
...
int CopyFile(char *pDest, char *pSrc, CopyProgressCallBackT pCallBack)
{
	...
	fdDest = open(pDest, O_WRONLY | O_CREAT | O_TRUNC, 0644);
	if (fdDest <= 0)
	{
		close(fdSrc);
		printf("Unable to open file %s\n", pDest);
...
//    assert(0);
	...
	_AudsrvMix = new AudsrvMixBuffer(32000);
...
static void _MainLoopSetSampleRate(Uint32 uSampleRate)
{
    _AudsrvMix->SetSampleRate(uSampleRate);
    {
        struct audsrv_fmt_t format;
        format.bits = 16;
        format.freq = uSampleRate;
        format.channels = 2;
        audsrv_set_format(&format);
        audsrv_set_volume(MAX_VOLUME);
    }
